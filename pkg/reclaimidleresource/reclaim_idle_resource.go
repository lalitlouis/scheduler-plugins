...
var (
	_ framework.PostFilterPlugin = &ReclaimIdleResource{}
	_ preemption.Interface       = &ReclaimIdleResource{}
)

// ReclaimIdleResource is a PostFilter plugin implements the preemption logic.
type ReclaimIdleResource struct {
	fh                  framework.Handle
	args                config.ReclaimIdleResourceArgs
...
func (pl *ReclaimIdleResource) Name() string {
	return Name
}

// New initializes a new plugin and returns it.
func New(rawArgs runtime.Object, fh framework.Handle) (framework.Plugin, error) {
	args, ok := rawArgs.(*config.ReclaimIdleResourceArgs)
	if !ok {
		return nil, fmt.Errorf("got args of type %T, want *ReclaimIdleResourceArgs", args)
	}
...
func (pl *ReclaimIdleResource) PostFilter(ctx context.Context, state *framework.CycleState, pod *v1.Pod, m framework.NodeToStatusMap) (*framework.PostFilterResult, *framework.Status) {
	defer func() {
...
func ExemptedFromPreemption(
	...
	// cpu idle duration
	cpuIdleSecondsDuration := time.Duration(policy.CPUIdleSeconds) * time.Second

	// gpu idle duration
	gpuIdleSecondsDuration := time.Duration(policy.GPUIdleSeconds) * time.Second

	// Pod still under resource idle seconds, can be exempted
	if scheduledAt.Add(cpuIdleSecondsDuration).After(now) || scheduledAt.Add(gpuIdleSecondsDuration).After(now) || scheduledAt.Add(tolerationDuration).After(now) {
		return true, nil
	}
	...
	// check if avg cpu usage is greater than threshold
	cpuIdleSecondsStr := strconv.FormatInt(policy.CPUIdleSeconds, 10)
	cpuActualAvgUsage := utils.CalculateAverageUsage(constants.ResourceTypeCPU, victimCandidate.Name, victimCandidate.Namespace, prometheusServiceIP, cpuIdleSecondsStr)

	klog.Info("Resource type ", constants.ResourceTypeCPU, " with idle seconds ", policy.CPUIdleSeconds, " and idle usage threshold of ", policy.CPUIdleUsageThreshold, " has average usage of ", cpuActualAvgUsage)

	if cpuActualAvgUsage > policy.CPUIdleUsageThreshold {
		return true, nil
	}

	// check if avg gpu usage is greater than threshold
	gpuIdleSecondsStr := strconv.FormatInt(policy.GPUIdleSeconds, 10)
	gpuActualAvgUsage := utils.CalculateAverageUsage(constants.ResourceTypeGPU, victimCandidate.Name, victimCandidate.Namespace, prometheusServiceIP, gpuIdleSecondsStr)

	klog.Info("Resource type ", constants.ResourceTypeGPU, " with idle seconds ", policy.GPUIdleSeconds, " and idle usage threshold of ", policy.GPUIdleUsageThreshold, " has average usage of ", gpuActualAvgUsage)

	if gpuActualAvgUsage > policy.GPUIdleUsageThreshold {
		return true, nil
	}
...
// The only difference is that it takes ReclaimIdleResource annotations in
// PriorityClass resources into account for selecting victim pods.
func (pl *ReclaimIdleResource) SelectVictimsOnNode(
	ctx context.Context,
...
func (pl *ReclaimIdleResource) GetOffsetAndNumCandidates(numNodes int32) (int32, int32) {
	return rand.Int31n(numNodes), pl.calculateNumCandidates(numNodes)
}

func (pl *ReclaimIdleResource) CandidatesToVictimsMap(candidates []preemption.Candidate) map[string]*extenderv1.Victims {
...
func (pl *ReclaimIdleResource) calculateNumCandidates(numNodes int32) int32 {
...
func (pl *ReclaimIdleResource) PodEligibleToPreemptOthers(pod *v1.Pod, nominatedNodeStatus *framework.Status) (bool, string) {
